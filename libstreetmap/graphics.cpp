...
#define ZOOM_FACTOR  1.4 /* For zooming on the graphics */
#endif
...
static const vector<t_color> predef_colors = {
    ...
    t_color(233, 229, 220)     //background colour, "BGColor"
};
...
//    
//    
//    //experimental values for now
//    //x11_state.colormap_to_use
//    double_buffer = XCreatePixmap(x11_state.display, x11_state.toplevel, width, height, x11_state.colormap_to_use);
//    
//    XSetBackground(x11_state.display, x11_state.current_gc, BlackPixelOfScreen(DefaultScreenOfDisplay(x11_state.display)));
//    //load the pix map
//    /*XCopyArea(display, bitmap, win, gc,
//          0, 0,
//          bitmap_width, bitmap_height,
//          100, 50);*/
//    //XSetWindowBackgroundPixmap(x11_state.display, x11_state.toplevel, p);
//    
//    XCopyArea(x11_state.display, double_buffer, x11_state.toplevel, x11_state.current_gc, 0, 0, width, height, 0, 0);
//    sleep(1);
//    XFlush(x11_state.display);
//    XFreePixmap(x11_state.display, double_buffer);
...
//            yworld_to_scrn(y1), xworld_to_scrn(x2), yworld_to_scrn(y2));
        ...
        XDrawLine(x11_state.display, double_buffer, x11_state.current_gc, xworld_to_scrn(x1),
            yworld_to_scrn(y1), xworld_to_scrn(x2), yworld_to_scrn(y2));
...
//            x11_state.current_gc, xl, yt, width, height);
        ...
        XDrawRectangle(x11_state.display, double_buffer,
            x11_state.current_gc, xl, yt, width, height);
...
//            x11_state.current_gc, xl, yt, width, height);
        ...
        XFillRectangle(x11_state.display, double_buffer,
            x11_state.current_gc, xl, yt, width, height);
...
//            (int) (startang * 64), (int) (angextent * 64));
        ...
        XDrawArc(x11_state.display, double_buffer, x11_state.current_gc, xl, yt, width, height,
            (int) (startang * 64), (int) (angextent * 64));
...
//            (int) (startang * 64), (int) (angextent * 64));
        ...
        XFillArc(x11_state.display, double_buffer, x11_state.current_gc, xl, yt, width, height,
            (int) (startang * 64), (int) (angextent * 64));
...
//            transpoints, npoints, Complex, CoordModeOrigin);
        ...
        XFillPolygon(x11_state.display, double_buffer, x11_state.current_gc,
            transpoints, npoints, Complex, CoordModeOrigin);
...
//        XftDraw* buff = XftDrawCreateAlpha(x11_state.display, double_buffer, wa.depth);
//        
        XftDrawString8(
...
float left_align_draw_text(float xc, float yc, const std::string& str_text, float boundx, float boundy){
    ...
    drawtext(xc+wrld_width/2, yc, str_text, boundx, boundy);
...
flushinput(void) {
    if (gl_state.disp_type != SCREEN)
        return;
#ifdef X11   
    XCopyArea(x11_state.display, double_buffer, x11_state.toplevel, x11_state.current_gc, 0, 0, wa.width, wa.height, 0, 0);
    
    XFlush(x11_state.display);
    XSetForeground(x11_state.display, x11_state.current_gc, x11_convert_to_xcolor(predef_colors[BGColor]));
    XFillRectangle(x11_state.display, double_buffer, x11_state.current_gc, 0, 0, wa.width, wa.height);
    XCopyArea(x11_state.display, double_buffer, x11_state.toplevel, x11_state.current_gc, 0, 0, wa.width, wa.height, 0, 0);
#endif
}
...
handle_zoom_in(float x, float y, void (*drawscreen) (void)) {
    ...
    float area = abs(current_screen.get_height() * current_screen.get_width());
        ...
        if (drawscreen != NULL) {
            drawscreen();
            //XCopyArea(x11_state.display, clear_buffer, x11_state.toplevel, x11_state.current_gc, 0, 0, wa.width, wa.height, 0, 0);
            XCopyArea(x11_state.display, double_buffer, x11_state.toplevel, x11_state.current_gc, 0, 0, wa.width, wa.height, 0, 0);
...
static void x11_init_graphics(const char *window_name) {
    ...
    XGetWindowAttributes(x11_state.display, root_window, &wa);
    
    double_buffer = XCreatePixmap(x11_state.display, root_window, wa.width, wa.height, wa.depth);
    //initialize the clear buffer with a specific colour

    /***************************************************************************/
    

    //change this back later this is for testing purposes
    x11_state.toplevel_draw = XftDrawCreate(
        x11_state.display,
        double_buffer, //original was x11_state.top_level
        x11_state.visual_info.visual,
        x11_state.colormap_to_use
        );
...
//        XCopyArea(x11_state.display, double_buffer, x11_state.toplevel, x11_state.current_gc, 0, 0, wa.width, wa.height, 0, 0);
//        
//        XSetForeground(x11_state.display, x11_state.current_gc, WhitePixelOfScreen(DefaultScreenOfDisplay(x11_state.display)));
//        XFillRectangle(x11_state.display, double_buffer, x11_state.current_gc, 0, 0, wa.width, wa.height);
                ...
                if (report.xbutton.window == x11_state.toplevel) {
                    ...
                    switch (report.xbutton.button) {
                        ...
                        case Button4: /* Scroll wheel rotated forward; screen does zoom_in */
                            ...
                            if(insideBox(x,y,window_size)){
                                if(directiveVectorOffset>0)
                                    directiveVectorOffset--;
                                drawscreen();
                                XCopyArea(x11_state.display, double_buffer, x11_state.toplevel, x11_state.current_gc, 0, 0, wa.width, wa.height, 0, 0);
                            }
                            else
...
bool insideBox(float x, float y, t_bound_box box) {
